...
1. 先完成 PPT “L08”决策页的第一次签字，不看参考答案。
2. 打开 [FlowERP-AI研发工作台.code-workspace](../FlowERP-AI研发工作台.code-workspace)，运行“课程/02 查看本讲合同”，讲次选择 `8`。
...
本讲不是阅读现成 Workflow。学员领取 L08 原子预占缺陷候选和一条会吞掉非零退出码的故障流水线，必须亲手交付：
...
开工前必须具备：可推送的训练分支、真实独立 Runner、可下载 Artifact、L08 原子预占缺陷候选、签字验收项和故障 Workflow。缺少远端平台时记录为“待补实验”，不能用讲师代跑、截图或另一个本地目录冒充达成。
...
- **不在 L08 新增第二项功能**：库存扩展、采购、API、页面一律退回。
- **Run C 是唯一产品交付**：冻结 Workflow/Harness/Eval 后，依据签字验收项完成最小原子预占修复；不按业务代码量得分。
...
同一份 L08 原子预占缺陷代码先被坏门禁判成绿色。学员必须把“CI 颜色”降级为待核验信号：先用 Artifact 证明 Run A 假绿，再只修 Workflow 得到 Run B 诚实红，最后冻结裁判、完成最小业务修复得到 Run C 可信绿。本讲建立远程独立复验，不自动合并，也不自动修复。
...
- 从 L08 原子预占缺陷候选创建 `lab/l08-<student-id>`；
...
### Ticket 4｜冻结裁判，完成最小产品修复 Run C

- 不修改 Workflow、Harness、Eval、等级和 expected；
- 依据签字验收项实现整单原子预占，不扩展第二项 ERP 功能；
...
| 维度 | 优秀 | 达成 | 发展中 | 未达成 |
|---|---|---|---|---|
| 假绿与因果隔离 | A/B/C 完整，C 只改 L08 签字业务写集 | 三段结论和文件边界成立 | 有三次 Run 但边界证据不足 | 只有最终绿灯、扩建 ERP 或改裁判求绿 |
