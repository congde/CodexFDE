...
> **基础线唯一性**：本讲只把“库存可用量导出”固化为 `REQ-COURSE-L03` 可验收 Spec，并实现最小结构解析器；不写 FlowERP 导出功能，不用补货案例替换基础任务。

</details>

## 连续案例｜第一幕：三个人口中的“可用库存”

**时间**：周二 09:10　**地点**：仓储晨会

越权改采购单的请求被拒绝后，周岚换了一个方案：“那先把可用库存导出来，我发给供应商和销售，大家按这个数协调。”这句话听起来比直接改账安全得多，陈予甚至说半小时就能写完 CSV。

可会议刚开始，三个口径就出现了。销售认为“在库 8 台就是可用 8 台”；仓库认为已经被订单预占的不能再承诺；采购希望导出中显示在途 12 台，方便供应商确认；财务又提醒在途不等于已经拥有，不能与现货合并。大家都使用“可用”这个词，却在回答不同问题。

需求记录里没有使用者、截止时间、字段定义、排序规则和空库存行为。更没有人说明导出是给机器继续处理，还是给人临时查看。Agent 很快生成了一份完整 Spec，甚至主动增加了“预计到货日期”，但这个字段在现有数据模型里并不存在。

林默意识到，写得完整不代表写得真实。此刻最危险的不是漏掉一个字段，而是把未经确认的猜测变成验收标准；一旦进入后续 Eval，它就会获得“业务事实”的外观。

> **决策时刻**：请把原始请求拆成“已确认事实、需要追问、明确不做、拒绝猜测”四栏。然后回答：如果今天只能冻结一个最小版本，谁使用、导出什么、按什么顺序、空集怎样表示？先签署判断，再阅读参考 Spec。

本讲结束时，`REQ-COURSE-L03` 应是一份可以被反驳和验收的合同，而不是最漂亮的需求文档。它会在下一讲成为 Agent 的授权边界：Spec 中每一个模糊词，都可能变成一次越界修改。

## 图文学习导航

### 先进入现场：一句“把库存导出来”至少藏着六个未决问题

![工作台把自然语言需求生成结构化 Spec](./assets/web-22-delivery-generated-spec.png)

*观察重点：生成了 Spec 不等于 Spec 正确。需求来源、使用者、库存口径、字段、排序、空集和失败语义都必须能被提问、拒绝和复验。*

**先别看答案**：先独立写出“可用库存”的计算口径，并列出一个你拒绝替用户猜测的字段。与同伴交换后，只允许用来源证据消解歧义，不以多数投票创造业务事实。

本讲按“原始请求 → 歧义标注 → 价值/风险判定 → 用例矩阵 → Spec Review”学习。用 [GOV.UK Discovery](https://www.gov.uk/service-manual/agile-delivery/how-the-discovery-phase-works) 检查是否先理解用户、问题和约束，用 [CMU Course Alignment](https://www.cmu.edu/teaching/assessment/basics/alignment.html) 检查每个验收项是否对应可观察结果；L03 不偷跑实现。

## 学员正文｜不要让一份漂亮 Spec 替业务撒谎

### 09:12：会议刚开始，答案已经有三个

周岚想把导出发给销售，所以她说的“可用”是不再被其他订单占用的数量；陈予要拿导出安排仓库，所以他还关心库位和批次；采购希望看见在途 12 台，方便继续追单。三个人都没有故意混淆需求，只是各自在解决不同问题。

林默把“增加库存导出功能”写在白板中央，没有马上打开 Spec 模板。他先问：谁在什么时刻用这份结果作什么决定？如果答不出来，新增字段只会让歧义藏得更深。

| 原话 | 当前判断 | 需要的证据或决定 |
|---|---|---|
| “把可用库存导出来” | 目标方向已知，口径未知 | 确认 `available` 是否等于 `on_hand - reserved` |
| “把在途也放进去” | 新候选需求 | 确认是否存在权威在途字段以及本次使用者 |
| “最好直接生成 Excel” | 方案偏好 | 先确认机器消费还是人工查看，基础线不预设格式扩张 |

这张表刻意不写“正确答案”。需求工作首先保护未知状态，避免会议里声音最大的人把偏好变成事实。

### 09:25：先写非目标，再写功能

团队最终确认，基础版本服务于销售和仓库对当前可承诺库存的共同判断。字段只保留 `sku`、`on_hand`、`reserved`、`available`，按 SKU 稳定排序；空库存返回空集合而不是报错。预计到货、供应商、价格和页面按钮全部退出本次范围。

非目标不是项目管理附注。它会在 L04 成为 Agent 的禁止扩张证据。如果不写，通用报表框架、Excel 样式和在途预测都可能被解释为“更完整地实现需求”。

这里要主动保留一个看似合理的错误路线：Agent 自动添加 `expected_arrival_at`，并为它写了验收条件。字段具体、测试也容易编写，但仓库没有权威来源。若团队接受，Spec 会用精确格式掩盖虚构事实。

### 09:40：把口头同意改成能够失败的场景

林默不写“系统应正确导出库存”，而是写三个可观察场景。正常库存要证明计算口径和稳定排序；空集要证明输出结构不漂移；写出失败要证明错误被显式报告且不会改变业务账。每个场景都有输入、预期和证据格式。

随后让另一组扮演反方。他们不评价文笔，只寻找两类漏洞：实现者可以在不满足业务意图时仍通过的空隙，以及 Spec 要求了仓库根本无法证明的事实。争议必须回到来源，不靠投票解决。

### 10:00：让机器只检查结构，不替人决定价值

保存 `REQ-COURSE-L03` 后运行最小解析入口：

```powershell
.\.venv\Scripts\python.exe -X utf8 -m workbench.cli spec FDE_SPEC.md
```

解析通过只证明必需章节和结构存在，不证明“可用库存”口径经过业务确认。为了暴露这种边界，复制一份临时 Spec，删除“非目标”后再次运行。若解析器仍然接受，修的是工作台结构校验；若口径本身错误，回到评审会，不允许用代码补业务决定。

> **第二次签字**：在评审记录中逐条写下“接受、拒绝、待验证”。至少保留一个被拒绝的模型建议，并说明拒绝证据。然后重新签署目标、非目标、正常场景和失败场景。

### 10:15：合同落笔，业务账仍然不动

会议结束时，FlowERP 没有新增导出函数，这不是进度落后。团队得到了一份可被实现和反证的合同，也保住了没有来源的字段不进入系统。林默把 Spec 交给下一讲，却没有把整个仓库一起授权：合同决定终点，能力信封还要决定 Agent 能走多远、能碰什么。
...
- **PPT 引导决策**：使用 [PPT 逐讲决策脚本](./PPT逐讲决策脚本.md) 的“L03”四个镜头；在学生提交第一次选择、依据和最大风险前，不揭示后果页。
- **Markdown 承载教材**：本讲连续案例、图文导航与学员正文/核心章节负责查证和建模；学生必须保留第一次判断与证据驱动的第二次签字。
- **VS Code 完成行动**：打开 [课程工作区](../../course/FlowERP-AI研发工作台.code-workspace)，先运行“查看本讲合同”，再按 [L03 行动卡](../../course/tasks/L03-Spec驱动.md) 构造失败、完成受控修改并复验。
...
| 本讲不做 | 不选 FlowERP 实现方案，不写导出业务函数，不改库存，不用解析器通过代替业务评审 |
...
**主线坐标**：L02 已把长期业务禁区写进仓库，但“增加库存导出”仍可能被运营、仓库、财务和开发解释成四种产品。现在工作台必须围绕 `REQ-COURSE-L03` 把争议压成唯一合同。业务账保持不变；产品账新增一份有来源、目标、非目标、场景、验收和证据格式的库存导出 Spec。
...
### 产品化思考检查点 2：不是所有输入都应该进入 Spec

本讲基础任务仍按大纲为“库存导出”形成 Spec，不改变通过标准；但学生必须在写 Spec 前保留一张需求判定卡，区分：

```text
Signal：有人提出或系统观察到的原始信号
Opportunity：有证据支持、值得继续理解的问题
Decision：Build / Experiment / Defer / Reject / Stop
Spec：对已经决定交付的受控范围建立执行与验收合同
```

讲师提供一条预设方案式输入，例如“做一个 AI 自动补货按钮”。学生先改写为用户问题，检查来源、当前路径、影响、替代方案和待补证据，再决定进入 Build、Experiment、Defer 或 Reject。只有 `Build` 才生成完整 Spec；`Experiment` 生成假设、最低成本实验和停止条件；其余决定保留理由与复查触发器。结构化解析器只能证明 Spec 字段齐全，不能证明这件事值得做。
...
<details>
<summary>选学迁移：缺货—补货合同（不进入 L03 基础课堂与通过线）</summary>
...
<details>
<summary>选学案例续篇：把完整电商履约方向拆成需求层（不进入基础通过线）</summary>
...
这张图不是实施架构图，而是需求边界图。它故意不承诺某个框架、消息中间件或数据库表，却固定了必须存在的业务事实。任何人如果想加入真实平台 OAuth、多仓拆单、运费试算或财务凭证，必须新开需求；不能把它们藏进同一份 Diff。
...
| FlowERP 示例 | “增加库存导出” | 固定字段、权威 `available`、稳定排序、空集和写失败语义 |

```mermaid
flowchart LR
    U["用户 Prompt<br/>实现库存导出"] --> CB["Context Builder"]
    S["版本化 Spec<br/>REQ-COURSE-L03"] --> CB
...
一个高质量 Prompt 应引用 Spec，而不是复制并改写全部规则。例如：“执行 `REQ-COURSE-L03` 的库存导出切片；读取 `FDE_SPEC.md` 和根规则；只处理导出合同允许的写集；输出 Diff、命令和剩余风险。”如果 Prompt 与 Spec 冲突，不能让最后一句话悄悄改合同；应暂停、记录冲突并发起 Spec 变更。
...
当前基础需求使用 `REQ-COURSE-L03`。来源不是装饰字段，它让后续反馈、变更和回滚可以追溯。合格写法说明业务事件和原始问题；不合格写法是“老板说的”或“优化体验”。

### 4.2 目标：描述可观察结果

目标应写成：授权用户导出 `sku/on_hand/reserved/available`，`available` 使用权威库存口径，结果按 SKU 稳定排序；空集保留合法表头，特殊字符可解析，写失败不留下半成品。它没有规定必须写哪个函数或用哪个框架，却明确了系统最终状态。
...
第二个失败实验是把验收项改成“实现 `export_inventory` 函数”。解析器会通过，但语义评审必须退回，因为函数存在不代表字段、口径、排序和失败行为正确。这个实验让学员看到结构验证和语义验证之间的边界。

## 从模糊词到可测指标

| 模糊表达 | 问题 | 可验收改写 |
|---|---|---|
| “增加导出” | 导出对象、字段、格式和失败行为不清楚 | 授权用户导出固定字段；按 SKU 排序；空集有表头；写失败无半成品 |
...
例如，导出把 `on_hand` 写进 `available`：如果 Spec 已固定口径，这是实现缺陷；如果财务突然要求新增成本字段，这是范围变更，必须先更新 Spec，不能直接改代码迎合请求；如果文件正确但下载页面仍提供旧文件，则是证据链或读取模型问题。一次验收记录必须写明分类、负责人、回到哪一层以及重新验证的命令。
...
- [ ] 六个章节存在且非空；
- [ ] 来源编号可追溯；
- [ ] 覆盖授权导出、空集、稳定排序、特殊字符和写入失败，且结果口径唯一；
- [ ] 失败用例说明哪些状态保持不变；
- [ ] 删除章节时错误指出章节名；
- [ ] 解析器不执行编码；
...
| 正反验收 | 30 | 覆盖授权导出、空集、稳定排序、特殊字符、写失败和不留半成品 |
...
评审结束前，让另一组学员只看验收用例，不看目标文字，尝试复述产品行为。如果他们能准确说出导出对象、字段、`available` 口径、排序、空集、转义和失败后的不变状态，说明验收合同信息充分；如果不同小组对字段、顺序或失败产物得出不同结论，歧义仍然存在。

还可以进行“最小反证”练习：为每条验收写出一个最小错误实现，例如把 `on_hand` 当作 `available`、空集不输出表头、依赖数据库自然顺序、特殊字符不转义、写失败留下半成品。若验收无法区分正确与这些错误实现，它就不够有辨识力，需要增加结果和失败状态断言。

Spec 变更也要管理兼容性。若课程后来要求导出新增成本字段或把排序从 SKU 改为仓库+SKU，这不是“改一行测试”，而是业务合同变化。应修改来源或增加决策记录，评估权限、旧文件消费者和报表影响，更新 Eval，再执行先红后绿。对外 API 或持久化字段发生变化时，还要说明迁移和旧客户端行为。通过这种练习，学员会理解 Spec 不是写完即冻结的文档，而是可以演进、但每次演进都必须有理由和证据的版本化合同。
