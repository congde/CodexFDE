...
**主线坐标**：CI 已经证明候选不合格，但红灯本身没有授权“随便修”。工程账现在把报告压成短寿命、最小范围、带禁止事项的 Repair Task；业务账依然停在原子预占失败，不允许借修复之名扩展到采购或页面重做。

**这一讲把系统推进到哪里**：FlowERP 的原子预占缺陷被限制在正确业务切片；工作台获得从长日志提取失败签名、复现命令、允许写集和禁止事项的诊断能力；`repair-task.json` 能反向链接原报告并被下一会话执行，构成本讲证据。修复质量首先取决于问题被压缩得是否准确，而不是模型收到多少日志。
