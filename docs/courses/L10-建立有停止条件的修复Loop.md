...
**主线坐标**：Repair Task 已经限定“改什么”，Loop 开始限定“最多改几轮、用多少时间和 Token、看见什么证据后停止”。业务账仍不能被循环直接审批；工程账新增跨轮状态、失败记忆和非成功终态。

**这一讲把系统推进到哪里**：FlowERP 的缺陷修复可以自动重试，却不会因为循环失控扩大业务风险；工作台获得轮次、时间、用量、无进展签名和安全停止；两轮同签名后进入 `stopped_no_progress` 并保留历史，构成本讲证据。Loop 最值钱的能力不是“继续”，而是知道什么时候继续已经不再经济。
