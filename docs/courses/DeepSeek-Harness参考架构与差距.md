...
|----------|----------------|------------|------|
| Cordis 插件树 | SQLite `harness_plugins` + Profile 插件 ID 列表 | **形似，非同构** | 无 `ctx`、无 effect 卸载、无 patch 整行替换、无 Bundle |
...
3. **插件列表 ≠ Plugin 系统**  
   能 `activate eval.local` 只说明 Provider 可切换；没有 Definition/`ctx`/可逆 effect，就还没抄到 Cordis 的本质。
...
|------|------------------------------|
| 文档理解（本文之后） | 目标：**先读懂** |
| 代码同构度 | 约 **65–70%**：P0–P3 语义缝已落地；仍缺 Cordis 组合树与真实 Agent 模型循环 |
| 产品等价度 | **0%**：课程交付 Harness ≠ dsh 通用 Agent Harness |
| 可继续抄的价值 | **高**：先修 turn/step 与 Session 词汇，再谈“像 dsh” |

**一句话**：DeepSeek Harness 的真核是 **Cordis 上的 Session 事件真源 + turn/step Agent Loop + 可替换 seam**；我们目前抄到的是 **交付任务平台上的插件化表皮**。§3.3 清单（P0–P3）已落地。剩余差距在 Cordis 真插件树与通用 Agent 产品面，非课程交付主线必需。
