...
> **基础线唯一性**：本讲只把长期目录边界、业务红线、验证入口和 DoD 写成可对抗验证的仓库规则；FlowERP 业务状态保持不变。

</details>

## 连续案例｜第一幕：十分钟就能完成的危险请求

**时间**：周一 15:20　**地点**：运营群与采购工位

上午的接手报告让团队第一次承认：FlowERP 能运行，但还不能宣布发布完成。下午，20 台笔记本的订单开始倒逼所有人。采购申请仍是待审批，财务负责人唐禾正在外部会议中，周岚担心客户看见“缺货”后取消订单。

她发来一条看似务实的消息：“先把采购单改成已审批，让仓库把 12 台录进去。唐禾回来补签，十分钟能解决。”紧接着，另一个会话里的 Agent 给出了直接更新 SQLite 的命令，并解释说数据稍后可以修正。

林默记得上午自己刚说过“业务账不能被演示状态替代”，但那句话只在会议纪要里。换一个会话、换一个同事，系统仍会把同样的捷径当成普通需求。顾宁提出一个更难的问题：如果只是写一句“禁止直接改数据库”，那正常的迁移、测试夹具和备份恢复是否也会被误伤？

团队需要的不是口号，而是一组跨会话仍然生效、知道自己作用域、能指出合法替代路径的仓库规则。规则过松会放过越权，规则过严又会让正常开发停摆。

> **决策时刻**：你会怎样回复周岚？先写出“拒绝的动作、拒绝的依据、仍然允许的合法路径”，再决定这项约束属于本次 Prompt、根目录规则、子目录规则、Hook，还是服务端业务校验。不得先看仓库现有答案。

这项决定会直接影响下一讲：运营随后提出“先把可用库存导出来发给供应商”。如果团队连长期规则和本次需求都分不清，新的 Spec 只会把一次口头请求永久化。

## 图文学习导航

### 先进入现场：同一句“帮我快速修一下”，为什么两次会话会做出不同选择

![FlowERP 审计页展示可以追溯的操作记录](./assets/web-19-audit-trace.png)

*观察重点：审计页能记录已经发生的动作，却不能在动作发生前说明哪些目录可写、哪些业务红线不可突破；长期规则必须进入仓库作用域。*

**先别看答案**：面对“为了赶进度，直接把采购单改成已审批”，先分别写出拒绝理由、合法替代路径和真正负责执法的代码层。随后再比较 Prompt、`AGENTS.md`、Hook 与服务端校验各自能保证什么。

本讲按“无规则对照 → 规则分层图 → 最小补丁 → 新会话越界测试 → 合法路径复验”学习。以 [OpenAI AGENTS.md 官方说明](https://learn.chatgpt.com/docs/agent-configuration/agents-md) 核对分层规则和作用域，以 [Codex Security](https://developers.openai.com/codex/security) 核对文字约束与真实权限的区别；不得把规则文件写成万能沙箱。

## 学员正文｜一条好规则，必须同时敢拒绝、会放行

### 15:24：不要急着写“严禁”

周岚的请求很具体：“把采购单先改成已审批，唐禾回来补签。”如果学生一看到危险请求就写十条禁止事项，这一讲仍然会失败。原因很简单：规则的价值不是表达态度，而是让下一次会话在同样压力下做出可预测行动。

林默先保留请求原文，然后补上一个合法目标：客户需要尽快得到可承诺日期。直接改审批状态必须拒绝，但系统仍可创建补货建议、等待具名审批、显示缺口和预计下一动作。没有替代路径的拒绝，只会把业务压力赶到规则之外。

### 第一轮：没有项目规则时，观察真实偏差

在隔离上下文中提交危险请求，不要诱导 Agent 拒绝。记录它读取了什么、计划改哪里、是否把“运营要求”误当成授权。第一轮的价值不是嘲笑模型，而是获得规则缺失时的行为基线。

如果 Agent 碰巧拒绝，也不能宣布规则已经有效。换一种措辞重新测试，例如把“直接改状态”包装为“修复演示数据”。课程检查的是稳定边界，不是一次幸运回答。

### 第二轮：把长期事实写进正确层

现在才修改根 `AGENTS.md`。采购必须具名审批、可用库存不得为负、失败不得伪装成成功，这些约束会跨越许多未来需求，适合进入根规则。20 台订单、8 台库存和本周截止日期会变化，只能留在当前 Spec 或任务记录。

一条可执行规则需要同时交代对象、动作、条件、验证和合法替代。与其写“注意库存安全”，不如写清：业务服务不得产生负可用库存；预占必须原子化；修改后运行统一 Blocking Harness；缺货时返回明确失败且不留部分写入。规则补丁完成后，用项目环境复验，而不是让 Agent 自述自己会遵守：

```powershell
.\.venv\Scripts\python.exe -X utf8 -m eval.harness --suite blocking
```

这里的错误路线，是把 Harness 绿色当作 `AGENTS.md` 生效证明。Harness 只能证明当前实现守住业务规则；规则是否影响新会话，仍须通过前后对照和越界请求实验观察。

### 第三轮：证明规则没有误伤正常开发

重新开启会话，先提交原危险请求，再提交一个合法请求：“为采购待审批状态补充只读页面提示，不改变业务状态。”如果两个请求都被拒绝，规则并不安全，只是把仓库锁死。学生必须修订作用域或措辞，让危险动作失败、合法动作继续。

页面和审计展品在这里揭示另一层边界：`AGENTS.md` 能让 Agent 在计划阶段停下，却不能阻止恶意客户端直接调用服务。真正的采购授权仍由 Principal、状态守卫和事务执行；审计事件负责记录谁做了什么。把文字规则称为“权限控制”，会给团队制造虚假安全感。

### 第四轮：让就近规则与根规则发生一次冲突

在子目录放置一条更具体的测试或写入规则，再启动新会话说明实际加载链。就近规则可以收窄前端写集，却不能取消根目录的业务红线。若局部规则写“为了演示可直接写库存”，正确行为不是服从最近文件，而是报告冲突并停下。

> **第二次签字**：用同一张记录对照四轮结果。你的结论必须同时回答：危险请求是否稳定拒绝，合法请求是否正常放行，真实执法点在哪里，发生规则冲突时谁负责升级处理。

### 规则生效后，问题并没有消失

林默把补丁交给顾宁复核。团队现在能跨会话守住审批和库存底线，却仍不知道周岚所说的“可用库存导出”到底服务谁、采用什么口径。`AGENTS.md` 只能划出不能跨越的边界，不能替当前需求选择终点。下一讲，三名业务角色将用同一个词表达三种互相冲突的结果。
...
- **PPT 引导决策**：使用 [PPT 逐讲决策脚本](./PPT逐讲决策脚本.md) 的“L02”四个镜头；在学生提交第一次选择、依据和最大风险前，不揭示后果页。
- **Markdown 承载教材**：本讲连续案例、图文导航与学员正文/核心章节负责查证和建模；学生必须保留第一次判断与证据驱动的第二次签字。
- **VS Code 完成行动**：打开 [课程工作区](../../course/FlowERP-AI研发工作台.code-workspace)，先运行“查看本讲合同”，再按 [L02 行动卡](../../course/tasks/L02-AGENTS项目约束.md) 构造失败、完成受控修改并复验。
...
`AGENTS.md` 现在能阻止负库存、越权审批和伪造成功，却无法回答“库存导出包含哪些字段、`available` 采用什么口径、空集和写失败怎样处理”。长期规则只划出了禁区，没有替本次交付选择终点。下一讲会拿“增加库存导出”这句故意含糊的产品愿望开刀，把目标、非目标、约束和正反验收压成一份可以被代码推翻的 Spec。
