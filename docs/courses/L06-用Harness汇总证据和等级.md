...
> 课程建设状态说明：本讲 Harness、报告 Schema、活动与评价规则属于已经形成的课程设计；学生预测、三类运行记录、具名裁决和达成数据属于待真实开课采集。参考仓库终态全绿不能替代学生对分级、异常和退出码的判断。

> **基础线唯一性**：本讲交付可用库存口径，并把 L05 及相关库存 Eval 汇入唯一 Harness；不自动触发、不自动修复，也不复制第二套质量判决。

</details>

## 连续案例｜第二幕：五份报告，三个答案

**时间**：周三 13:50　**地点**：发布评审会

重复入库已经修复。林默准备合并时，把现有验证结果投到屏幕上：单元测试显示通过，幂等 Eval 显示通过，一个旧脚本仍然报告库存口径异常，JSON 文件没有更新时间，CI 尚未运行。周岚只看到大部分是绿色，认为可以发版。

顾宁随机请三个人只根据现有材料回答“是否可交付”。三个人给出三个答案：可以、不可以、无法判断。问题不在他们缺乏技术能力，而在每个检查使用不同名称、等级、输出格式和退出码；有的失败只写进文本，进程仍返回 0。

团队可以再写一个汇总页面，但如果页面重新计算业务规则，就会出现第二套裁决；也可以简单统计绿色数量，但一个阻断级不变量不应被十个普通检查投票淹没。Harness 必须汇总已有 Eval，并让人和机器得到同一个结论。

唐禾提出最后一道质问：“如果库存口径错了但页面显示九项通过、一项失败，你们准备把财务风险定为什么等级？谁签字？”

> **决策时刻**：请先给现有检查分级，并定义文本、JSON、退出码如何表达同一个总决策。另一组同学只能读取机器报告；如果他们得不到与你相同的结论，你的 Harness 设计尚未完成。

统一裁决解决了“大家看见不同答案”的问题，却没有解决“大家会不会记得运行”。下一次缺货订单测试中，一名开发者在本地提交前忘了执行 Harness，部分业务状态已经写入数据库。

## 图文学习导航

### 先进入现场：五个检查都跑过，团队却仍然没有一个共同结论

![工作台证据区域展示可追溯的 Eval 结果](./assets/web-07-delivery-evidence.png)

*观察重点：单项测试、文本日志、JSON 报告和进程退出码必须表达同一个裁决；只要其中一个仍显示成功，自动化链路就可能把失败伪装成通过。*

**先别看答案**：给“库存口径错误但普通测试通过”定级，并说明它是否阻断交付。然后让另一组只看机器报告，检验他们能否得到相同结论。

本讲按“分散检查 → 统一注册 → 分级裁决 → JSON/退出码对照 → 反向造假测试”学习。用 [OpenAI Evals Guide](https://developers.openai.com/api/docs/guides/evals) 理解评价任务与运行结果，用 [Google SRE Monitoring](https://sre.google/sre-book/monitoring-distributed-systems/) 区分信号、症状和根因；Harness 只汇总，不复制领域规则。

## 学员正文｜五盏灯不能靠投票决定发布

林默把五份报告排在桌面上时，会议室里没有人缺少数据，缺的是共同语言。单元测试写着 `OK`，幂等检查是绿色，旧脚本说库存口径异常，另一个扫描器只输出一行 warning，最后一份 JSON 甚至没有本次提交的 SHA。周岚数了数：“三绿一黄一红，绿的多，先发吧。”这句话听起来务实，却把业务损失交给了多数票。

先不要急着写 Harness。请在纸上写下三件事：哪一类失败必须阻断；哪一类失败只提醒但不改变放行；检查程序自己崩溃时，系统究竟知道了什么。这里最容易出现的错误路线，是把所有结果换算成分数，例如 80 分以上通过。库存非负、重复入库和采购审批不是可以被平均掉的题目；一个阻断不变量失败，即使另外九十个格式检查通过，也不能发布。

林默先查看机器合同，再运行唯一入口：

```powershell
.\.venv\Scripts\python.exe -X utf8 -m workbench.cli course-contract --lesson 6
.\.venv\Scripts\python.exe -X utf8 -m eval.harness --suite all
```

他没有先看终端最后一行，而是同时记录 case 级结果、总 `decision`、进程退出码和报告中的提交身份。四者必须能互相推出：blocking 失败对应 `block` 和非零退出码；observing 失败可以形成告警，但不能伪装成 case 通过；用例异常说明“没有得到结论”，更不能被捕获后改写为绿色。Harness 的价值不是把日志变漂亮，而是让人、Hook、CI 和后续 Loop 读取同一个判决对象。
...
- 第一行只统计阻断级失败，所以一个 blocking 失败不能被其他绿色抵消。
- 第二行单独保留观察级失败；它会进入报告，但不偷偷改变阻断合同。
- 第三行让人和机器读取同一个总决定，不再各自解释日志。
- 最后一行把总决定投影为进程退出码；若 JSON 写着 `block` 而进程返回 0，Harness 自己就不可信。

接着，小组主动制造三种信号。第一组让 observing 用例失败，另一组只看 JSON 判断能否放行；第二组破坏库存非负用例，检查终端与 JSON 是否同时阻断；第三组让用例抛出异常，验证报告是否保留 `error`，而不是用宽泛的 `except` 吞掉。最后还要让报告目录不可写：如果屏幕显示全绿但证据无法落盘，这次运行仍不能成为交付证据。

**看似合理的错误路线**是给每个消费者一段“智能判断”：Hook 自己统计失败数，CI 自己解析文本，Web 再重新计算一次颜色。短期很快，长期会产生三套裁判。正确边界是 Harness 负责发现、执行、分级和形成稳定报告；领域规则仍在原 Eval；消费者只读取稳定字段，不重新解释业务。

**第二次签字**：现在重新回答“这次是否可发”。你的结论必须引用报告身份、blocking 数量、decision、退出码和未执行项。若另一组只拿到 JSON 仍不能得到同一结论，说明你交付的是一份报告，不是一个质量合同。

离场时，FlowERP 多了统一的可用库存口径，工作台多了唯一裁决入口。可新的风险也随之出现：唯一入口仍靠人主动运行。下一讲，团队要把它接到最容易忘记检查的时刻——Agent 准备宣布完成之前。
...
- **PPT 引导决策**：使用 [PPT 逐讲决策脚本](./PPT逐讲决策脚本.md) 的“L06”四个镜头；在学生提交第一次选择、依据和最大风险前，不揭示后果页。
- **Markdown 承载教材**：本讲连续案例、图文导航与学员正文/核心章节负责查证和建模；学生必须保留第一次判断与证据驱动的第二次签字。
- **VS Code 完成行动**：打开 [课程工作区](../../course/FlowERP-AI研发工作台.code-workspace)，先运行“查看本讲合同”，再按 [L06 行动卡](../../course/tasks/L06-Harness证据汇总.md) 构造失败、完成受控修改并复验。
...
**主线坐标**：L05 已交付幂等入库并留下一个有辨识力的 Eval；现在 FlowERP 还需要统一“在库—预占=可用”的读取口径，工程账则要把幂等、非负与可用库存检查汇成唯一结论。Harness 只能读取和裁决，聚合异常不得改变业务账。

**这一讲把系统推进到哪里**：FlowERP 获得唯一的可用库存口径，并与 L05 幂等入库、库存非负检查一起进入统一 Harness；工作台获得唯一阻断入口、等级、结构化报告和可信退出码。三项核心结果、一个 observing 对照和异常路径共同构成本讲证据。Harness 的价值是让 Hook、CI、Loop、Graph 和人审不再各说一套“通过”。
