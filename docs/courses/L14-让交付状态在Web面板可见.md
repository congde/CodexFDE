...
> 课程建设状态说明：本讲页面、状态文案、走查脚本和评价规则属于已经形成的课程设计；学生首次判断、四方对账、陌生人走查和达成数据属于待真实开课采集。参考截图、静态页面或讲师演示不能替代学生证明页面与权威状态一致。

> **基础线唯一性**：本讲只完成一个真实 FlowERP 操作主路径，并让 DOM、API、SQLite 与业务对象四方一致；静态假数据和陈旧绿灯均判定失败。

</details>

## 连续案例｜第四幕：昨天的绿色还留在今天的页面上

**时间**：第四周周二 08:55　**地点**：仓库操作台

陈予打开 FlowERP，驾驶舱显示库存同步正常，补货任务也是绿色。他点进任务详情，却发现 API 状态仍为 `review`；直接查询 SQLite，最后一条事件是昨晚的评测完成；采购业务对象则明确显示“待唐禾审批”。

页面没有完全造假。它读取的是一次成功演示留下的静态摘要，只是没有告诉用户数据时间和来源。对开发者而言，这是一处前端缺陷；对仓库人员而言，它可能导致未审批货物被安排入库。

林默让团队选定一条真实操作主路径，从页面按钮开始，沿 DOM、API、SQLite 和 ERP 业务对象逐层对账。每一层都要能回答同一个任务是谁、当前状态是什么、下一步由谁行动、失败时业务账是否改变。

周岚希望保留醒目的绿色卡片。顾宁同意，但要求成功不能只靠颜色表达，状态变化要能被辅助技术感知，刷新后也不能继续展示旧证据。

> **决策时刻**：在页面上任选一个关键数字或状态，先画出它的来源链，再逐层取证。只要四方有一处不一致，就必须把页面降级为“未知/待处理”，并显示责任人与下一步，而不是继续保留绿色。

页面终于成为可操作工作面，而不是演示墙。真实用户也开始提出反馈。其中一句“这个按钮不好用”很快被自动系统收录；如果不经过分诊，它可能直接变成新的阻断规则。

## 图文学习导航

### 先进入现场：漂亮面板上的绿色，可能来自昨天的缓存

![FlowERP 当前经营驾驶舱展示来自业务系统的实时状态](./assets/web-13-dashboard-live-data.png)

*观察重点：视觉完成度不能掩盖数据来源。每个关键数字和状态都要能沿 DOM → API → SQLite → ERP 业务对象反向追踪，失败后也要显示可行动信息。*

**先别看答案**：在浏览器里选一个库存数字，先猜它来自哪个接口、哪张表、哪个领域计算；再沿四层逐一对账。若任一层不一致，页面不得保留成功状态。

本讲按“用户主路径 → 四方对账 → 状态消息 → 失败可见 → 刷新与陈旧证据”学习。用 [W3C WCAG 2.2 Status Messages](https://www.w3.org/WAI/WCAG22/Understanding/status-messages.html) 检查状态是否可感知，用 [OpenTelemetry Signals](https://opentelemetry.io/docs/concepts/signals/) 理解事件、日志和链路信号如何关联；课程不要求引入外部观测依赖。

## 学员正文｜一个漂亮的旧绿色，比明确报错更危险

周岚打开工作台，看到补货任务仍是绿色，于是继续安排发货。几分钟后她才发现页面展示的是昨天缓存的结果，当前 API 已经不可用，数据库里的任务停在 rework，ERP 采购也没有完成审批。页面没有造假数据，却通过沉默让用户相信了旧事实。

Web 的质量不先取决于组件数量，而取决于它是否帮助用户做出正确下一步。加载、空态、失败、未知、旧数据必须有不同语义；状态必须带稳定 Task ID、更新时间和来源；用户要能从摘要下钻到 Spec、Eval 和具名事件。最诱人的错误路线是用定时器模拟 queued → executing → completed，让演示顺滑，却切断了页面与权威状态的联系。

启动真实 FlowERP，而不是打开静态 HTML：

```powershell
.\.venv\Scripts\python.exe -X utf8 main.py --host 127.0.0.1 --port 8000
```

创建一项真实交付任务，记录同一个 Task ID 在 DOM、Task API、SQLite 和 ERP 业务对象中的值。四方不必字段完全相同，但必须能解释映射：页面显示的状态来自哪个 API 字段；事件对应哪条数据库记录；产品结果映射到哪个订单、采购或库存对象。只截一张绿色页面，无法证明任何一致性。
...
- 没有更新时间时直接返回 `unknown`，页面不能猜一个友好的绿色。
- `age` 把“旧”变成可计算事实，而不是用户刷新后凭感觉判断。
- 只有仍在自动推进的状态才接受 300 秒新鲜度门槛；故意等待人审的 `review` 不会仅因等待较久被伪装成系统故障。
- 页面最终投影 `stale/current/final`，但 Task 业务状态仍来自服务端，前端不再创造第二套状态机。

随后进行七连测：首次加载；空列表；API 断开；返回未知状态；返回错误字段类型；任务进入 rework；服务恢复。页面不得沿用旧成功，不得把 `202` 显示为完成，也不能把错误吞成空态。错误文案要告诉用户当前不知道什么、最后可信时间和下一步，而不是只有“出错了”。再让另一位同学不听讲解完成一次查状态、看证据、具名审核或打回。

**看似合理的错误路线**是让前端在状态缺失时“智能推断”：有报告就显示完成，有百分比就画进度。推断会制造第二套状态机。页面只能投影服务端权威事实；需要衍生解释时，必须标注来源和不确定性。

可用性验收不能由开发者自己点击一遍完成。请找一名未参与实现的同学，给他一个明确目标：“判断这项补货能否继续，并找到阻断证据。”记录他第一次点击、误解的状态词、完成时间和求助点，再修订信息架构。页面是否漂亮可以讨论，是否诱导错误动作必须用行为证据判断。

还要在窄屏和键盘操作下复验同一任务，确认焦点顺序、状态文本和错误提示不依赖颜色或鼠标悬停。无障碍不是附加评分项；看不到证据或无法触发审核，会直接改变责任人能够作出的决定。

**第二次签字**：页面此刻是否允许用户继续发货？请引用同一 Task ID 的四方对账、数据新鲜度、错误态表现和可执行下一步。若答案只来自颜色，页面仍不可信。

页面终于说真话，也让真实用户暴露了新的裂缝：唐禾说“财务页面很难用”。一句现场抱怨还不是需求，更不能直接改写 blocking Eval。下一讲把反馈变成可治理的进化输入。
...
- **PPT 引导决策**：使用 [PPT 逐讲决策脚本](./PPT逐讲决策脚本.md) 的“L14”四个镜头；在学生提交第一次选择、依据和最大风险前，不揭示后果页。
- **Markdown 承载教材**：本讲连续案例、图文导航与学员正文/核心章节负责查证和建模；学生必须保留第一次判断与证据驱动的第二次签字。
- **VS Code 完成行动**：打开 [课程工作区](../../course/FlowERP-AI研发工作台.code-workspace)，先运行“查看本讲合同”，再按 [L14 行动卡](../../course/tasks/L14-Web状态面板.md) 构造失败、完成受控修改并复验。
...
| 高阶性 | 对 DOM、API、SQLite 和 ERP 业务对象进行四方对账，评价轮询新鲜度、错误恢复和密钥边界 |
| 创新性 | 把 UI 从美化作业转为可信状态投影实验，以真实错态驱动交互设计 |
| 挑战度 | 空、载入、失败、未知、陈旧和成功必须可区分；旧绿灯不能在断网后继续冒充当前状态 |
| 学生中心活动 | 学生提交真实任务、断网和注入未知状态，互换 Task ID 与业务 ID 完成四方对账 |
| 课程思政融入 | 页面不知道就显示未知、失败就显示失败，训练诚实表达、用户责任和数据安全意识 |
| 形成性评价 | 状态文案合同 + DOM/API/SQLite/业务对象对账 + 失败重试 + 无密钥检查 |
...
| 必须掌握 | 状态文案合同；DOM/API/SQLite/ERP 业务对象四方对账；空载错态；轮询新鲜度；前端安全与可访问性 |
| 能力判据 | 同一 Task ID 与业务 ID 在四层关键字段一致，断网、未知枚举和字段漂移都不会留下旧绿灯或吞错 |
...
**这一讲把系统推进到哪里**：业务人员能在同一页面审查 ERP 变更、质量结论和下一责任人；工作台获得忠实投影权威状态、展开真实 Diff 和结构化 evidence 的交互层；DOM、API、SQLite 与 ERP 业务对象四方对照构成本讲证据。Web 页当然是图，但它必须是一张由真实状态驱动、可下钻、不会撒谎的动态图。
...
- **工作台增量**：可信状态投影、证据下钻、轮询/重试、新鲜度、可访问性和四方对账；
...
### 产品化思考检查点 4：内部领域模型不能原样变成导航菜单

工作台内部可以拥有 Signal、PRD、RFC、Risk、Run、Eval、Release、Outcome 等细粒度对象，用户每天不应该逐个维护它们。本讲要求学生把内部对象压缩为不超过五个用户工作包：事项、决策简报、交付合同、证据包、结果复盘，并解释每个内部对象为什么显示、折叠或仅在异常时出现。

页面黄金路径是：输入一个事项 → 系统给出事实/假设和风险路径建议 → 人选择拒绝、实验或交付 → 查看当前合同和证据 → 只在阻断、例外、高风险发布和结果判断时被打断。陌生人走查除了原有状态五问，还要检查：能否在不学习全部术语的情况下找到“现在需要我决定什么”；如果需要讲师解释菜单，优先删减和重组信息架构，不增加说明卡片。
...
| L14-O2 | 用同一 Task ID 与业务 ID 对账 DOM、API、SQLite 与 ERP 业务对象 | 四方对账表 | 抽样字段全部一致 |
...
| 线下 34–50 分钟 | 用同一 Task ID 与业务 ID 对质 DOM、API、SQLite 与 ERP 业务对象 | 四方对账表 |
...
临时断开 API，若页面仍显示旧的“全部通过”且没有过期标记，说明它在使用静态或陈旧数据。正确页面应显示服务不可用或状态未知。
...
| 项目 | 分值 | 合格表现 |
|---|---:|---|
| 状态诚实与四方对账 | 30 | DOM、API、SQLite、ERP 业务对象一致，未知和离线不显示旧绿灯 |
