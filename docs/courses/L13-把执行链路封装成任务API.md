...
> **基础线唯一性**：本讲只通过最小 Task API 提交、查询并完成一个补货需求；`202 Accepted` 只表示已受理，成功必须由后续权威状态和证据证明。

</details>

## 连续案例｜第四幕：客户收到 202，以为十二台已经在路上

**时间**：第四周周一 09:30　**地点**：渠道联调群

渠道合作方第一次通过 Task API 提交补货需求。服务立即返回 `202 Accepted` 和 Task ID。对方工程师把响应截图发给客户经理，并在工单中写下“补货完成”。半小时后任务仍在评测，采购审批甚至还没开始。

双方都没有伪造数据：API 的确成功受理了请求，客户经理也确实看见了成功状态码。问题在于系统没有把“受理、执行、等待审核、业务完成”之间的责任界面讲清楚。进程一旦重启，内存中的任务状态还可能消失。

林默需要让每一次状态变化都有持久事件，让客户端能够凭 Task ID 查询权威状态。`accepted` 只能承诺系统接管了请求，不能承诺业务结果；`succeeded` 也必须绑定 Eval、具名审核和最终 ERP 状态。

顾宁又同时发起两个相同请求，并在运行中重启服务。若任务身份、幂等语义和恢复机制不稳定，API 只会把本地不确定性扩散给更多团队。

> **决策时刻**：请为 `accepted`、`running`、`review`、`succeeded`、`failed` 各写一条权威证据和客户端下一步动作。然后证明服务重启后，同一个 Task ID 仍能还原状态，重复提交不会悄悄执行两次。

Task API 让过程可以跨团队查询，却也让新的谎言变得可见：如果 Web 页面仍从静态数据或缓存读取，它会把昨天的绿灯展示给今天的用户。

## 图文学习导航

### 先进入现场：API 返回 202，客户为什么仍然不能认为补货已经完成

![交付工作台展示任务创建入口与可查询概览](./assets/web-20-delivery-task-live.png)

*观察重点：提交成功只创建了一个可追踪任务。客户端还要用 Task ID 查询状态事件、最终 Eval、失败详情和持久化业务结果。*

**先别看答案**：给 `accepted`、`running`、`review`、`succeeded`、`failed` 分别写出权威证据。若进程在受理后重启，哪些状态必须从 SQLite 恢复？

本讲按“提交 → Task ID → 状态查询 → 重启恢复 → 并发与错误语义”学习。用 [RFC 9110 的 202 Accepted](https://www.rfc-editor.org/rfc/rfc9110.html#name-202-accepted) 校准“已受理但未完成”，用 [RFC 9457 Problem Details](https://www.rfc-editor.org/rfc/rfc9457.html) 设计稳定错误结构；HTTP 码不代替业务裁决。

## 学员正文｜收到请求，不等于完成交付

运营系统提交 12 台补货请求，API 返回 `202 Accepted`。客户端把它翻译成“补货成功”，马上通知客户到货时间。此刻任务其实只写入队列，尚未执行 Harness，更没有采购批准。一个正确的 HTTP 状态，被错误的产品语义变成了业务承诺。

API 不是给 CLI 套一层 HTTP。它必须把长流程建模为可查询、可恢复、可审计的任务资源：创建请求返回稳定 Task ID；状态迁移受原子守卫控制；事件保留 actor 和证据；服务重启后仍能恢复事实。最常见的错误路线是把内存线程当任务系统，或让客户端根据进度百分比猜测完成。

先读取能力和课程合同，再启动服务：

```powershell
.\.venv\Scripts\python.exe -X utf8 -m workbench.cli course-contract --lesson 13
.\.venv\Scripts\python.exe -X utf8 -m workbench.cli serve --port 8000
```

行动课使用两个客户端提交相同请求。检查幂等键相同、载荷相同时是否返回同一任务；键相同、载荷变化时是否明确冲突。任务只能沿 queued → executing → evaluating → review → completed 等合法边前进，`202` 只说明已接收。重启服务后，再用 Task ID 查询事件链；如果状态只存在内存，不能称为协作 API。
...
- 第一条守卫把状态机放在服务端，客户端不能用一个请求跳过执行和评测。
- 进入 `completed` 前必须同时出现具名审核人和明确 `approve`。
- 最后一条再核对机器证据；只有人签字或只有绿灯都不够。
- `202` 发生在这些检查之前，所以它只能承诺“任务已受理”，不能承诺“交付已完成”。

随后制造非法迁移和并发竞争：两个 Worker 同时领取任务时只能有一个获得所有权；客户端不能从 queued 直接写 completed；失败和 rework 必须保留错误与证据，而不是统一返回 200。错误响应应表达类型、状态和可操作说明，但不能泄漏内部路径或凭据。

**看似合理的错误路线**是让 GET 接口现场重新计算“应该是什么状态”。任务状态必须来自持久化事件与受控迁移，实时读取只用于库存、审批等“此刻世界”的业务事实。混淆二者会让历史被当前值重写。

让同伴只使用 API 文档和一个 Task ID 完成复验：查询最后事件、定位 Harness 报告、尝试一次非法迁移，再重启服务确认任务仍存在。作者不得口头解释数据库表或后台线程。若同伴只能靠读取源码才能判断状态，API 合同还没有成为真正的协作边界。

**第二次签字**：客户端现在可以向用户承诺什么？请引用 HTTP 语义、Task 状态、最后事件、Harness 证据和人审状态。只有 completed 且证据链闭合时，才能称为交付完成。

任务 API 已经说真话，但绝大多数用户不会阅读 JSON。下一讲的挑战不是把数据画得漂亮，而是让页面在加载、失败、未知和旧数据时仍然不诱导错误决策。
...
- **PPT 引导决策**：使用 [PPT 逐讲决策脚本](./PPT逐讲决策脚本.md) 的“L13”四个镜头；在学生提交第一次选择、依据和最大风险前，不揭示后果页。
- **Markdown 承载教材**：本讲连续案例、图文导航与学员正文/核心章节负责查证和建模；学生必须保留第一次判断与证据驱动的第二次签字。
- **VS Code 完成行动**：打开 [课程工作区](../../course/FlowERP-AI研发工作台.code-workspace)，先运行“查看本讲合同”，再按 [L13 行动卡](../../course/tasks/L13-任务API.md) 构造失败、完成受控修改并复验。
