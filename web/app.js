...
function deliveryStatus(view) {
  const item=view&&view.status||{},owner=item.owner||{};
  return '<span class="status '+esc(item.code||"unknown")+'">'+esc(item.title||"未知状态")+'</span><span class="cell-sub">责任人：'+esc(owner.label||owner.id||"待确认")+'</span><span class="cell-sub">下一步：'+esc(item.next_action||"停止并核对状态")+'</span>';
}
...
async function renderDelivery() {
  const result=await Promise.all([api("/api/v1/delivery/views?limit=100"),api("/api/v1/feedback"),api("/api/v1/evolutions?limit=100"),api("/api/v1/delivery/capabilities"),api("/api/v1/course/status"),api("/api/v1/course/lessons")]);
  state.deliveryViews=result[0].items;state.tasks=state.deliveryViews.map(function(view){return view.task;});state.feedback=result[1].items;state.evolutions=result[2].items;state.courseStatus=result[4];state.courseLessons=result[5].items;
  ...
  const deliverySummary=result[0].summary||{},active=deliverySummary.active||0,passed=deliverySummary.completed||0,awaitingReview=deliverySummary.review||0;
  ...
  $("#delivery-tasks-table").innerHTML=table(["任务 / 需求","业务对象","权威状态 / 下一步","更新时间","操作"],state.deliveryViews.map(function(view){const x=view.task,code=view.status.code,allowed=view.allowed_actions||[];let actions='<button class="button small ghost" data-delivery="view" data-id="'+x.id+'">证据</button>';if(allowed.indexOf("run")>=0)actions+='<button class="button small primary" data-delivery="run" data-id="'+x.id+'">'+(code==="rework"?"重新执行":"执行与评测")+'</button>';if(allowed.indexOf("approve")>=0)actions+='<button class="button small primary" data-task-review="approve" data-id="'+x.id+'">接受</button><button class="button small danger" data-task-review="reject" data-id="'+x.id+'">打回</button>';if(allowed.indexOf("export_candidate")>=0)actions+='<button class="button small primary" data-course-export="'+x.id+'">导出候选</button>';return '<tr><td><b>'+esc(x.id)+'</b><span class="cell-sub">'+esc(x.requirement_id||"未关联需求")+' · '+(x.execution_mode==="codex"?"Codex 改码":"仅验证")+'</span><span class="cell-sub delivery-request">'+esc(x.request)+'</span></td><td>'+esc((x.business_refs||[]).join("、")||"—")+'</td><td>'+deliveryStatus(view)+'</td><td>'+dateTime(view.freshness.updated_at)+'<span class="cell-sub">'+esc(view.freshness.state||"unknown")+'</span></td><td><div class="row-actions">'+actions+'</div></td></tr>'; }));
  $("#delivery-feedback-table").innerHTML=table(["反馈 / 任务","来源","结论与证据","状态","审核人","操作"],state.feedback.map(function(x){const promoted=state.evolutions.some(function(e){return e.feedback_id===x.id;}),actions=x.status==="pending_review"?'<button class="button small primary" data-feedback-review="accept" data-id="'+x.id+'">接受</button><button class="button small danger" data-feedback-review="reject" data-id="'+x.id+'">驳回</button>':(x.status==="accepted"&&!promoted?'<button class="button small primary" data-evolution-promote="'+x.id+'">提升</button>':""),e=x.evidence||{},failure=(e.blocking_failures||[]).join("、")||e.error||"人工反馈";return '<tr><td><b>'+esc(x.id)+'</b><span class="cell-sub">'+esc(x.task_id)+'</span></td><td>'+esc(x.source)+'</td><td>'+esc(x.conclusion)+'<span class="cell-sub">失败证据：'+esc(failure)+'</span><span class="cell-sub">下一步：'+esc(x.next_step)+'</span></td><td>'+status(x.status)+'</td><td>'+esc(x.reviewed_by||"—")+'<span class="cell-sub">'+dateTime(x.reviewed_at)+'</span></td><td><div class="row-actions">'+actions+'</div></td></tr>'; }));
...
async function showDeliveryTask(taskId) {
  const view=await api("/api/v1/delivery/views/"+encodeURIComponent(taskId)),task=view.task;
  const summary=view.eval.summary||{},results=view.eval.results||[],spec=view.spec.content||{};
  ...
  const execution=view.execution.evidence||{},executionEvent=view.execution.available;
  ...
  const events=(view.events||[]).map(function(x){const evidence=x.evidence?'<code>'+esc(evidencePreview(x.evidence))+'</code>':'';return '<li><i></i><div><b>'+esc((x.status||{}).title||x.to_status)+'</b><span>'+esc(x.detail||"状态已更新")+'</span><small>'+esc(x.actor||"system")+' · '+dateTime(x.created_at)+'</small>'+evidence+'</div></li>';}).join("");
  ...
  $("#delivery-task-detail").innerHTML='<div class="detail-hero"><div class="detail-hero-top"><div><h3>'+esc(task.id)+'</h3><p>'+esc(task.request)+'</p></div><div>'+deliveryStatus(view)+'</div></div><div class="detail-grid">'+detailField("需求编号",task.requirement_id||"—")+detailField("业务对象",(task.business_refs||[]).join("、")||"—")+detailField("执行模式",task.execution_mode==="codex"?"Codex 受控改码":"只验证现有代码")+detailField("写入范围",(task.write_scope||[]).join("、")||"只读")+detailField("证据完整性",view.integrity.truthful?"一致":"需核对："+(view.integrity.issues||[]).join("、"))+detailField("数据新鲜度",view.freshness.state+" · "+(view.freshness.age_seconds==null?"未知":view.freshness.age_seconds+" 秒"))+detailField("决策",summary.decision||"尚未评测")+detailField("阻断通过",blockingPassed)+detailField("阻断失败",summary.blocking_failed==null?"—":summary.blocking_failed)+detailField("交付审核",task.reviewed_by?(task.reviewed_by+" · "+task.review_decision):"待具名审核")+detailField("反馈 / 进化",view.feedback.total+" / "+view.evolution.total)+'</div></div><div class="delivery-evidence-grid"><section><h3>结构化 Spec</h3>'+specHtml+'</section><section><h3>状态事件链</h3><ol class="event-chain">'+events+'</ol></section></div>'+executionHtml+'<section class="detail-section"><h3>Eval 结果</h3><div class="detail-lines">'+evalHtml+'</div></section><form id="delivery-feedback-form" class="delivery-feedback-form"><h3>记录结构化反馈</h3><div class="form-grid"><label class="form-field">来源<input name="source" required value="web-review"></label><label class="form-field">结论<input name="conclusion" required placeholder="例如：失败证据清晰"></label><label class="form-field">下一步<input name="next_step" required placeholder="例如：补充边界测试后重跑"></label></div><button class="button primary" type="submit">提交待审核反馈</button></form>';
...
async function monitorDeliveryTask(taskId) {
  ...
  for(let attempt=0;attempt<800;attempt+=1){
    await pause(750);
    try{
      const view=await api("/api/v1/delivery/views/"+encodeURIComponent(taskId)),code=view.status.code;
      if(code!==previous){previous=code;await renderDelivery();await showDeliveryTask(taskId);}
      if(["review","rework","failed","dead_letter","completed"].indexOf(code)>=0){
        if(code==="review")toast("Blocking Eval 通过，流水线已停在具名审核","success");
        else if(code==="rework")toast("Blocking Eval 未通过，任务已保留证据并退回返工","error");
        else if(code==="failed"||code==="dead_letter")toast("自动流水线失败，原因已写入事件链","error");
        return;
      }
